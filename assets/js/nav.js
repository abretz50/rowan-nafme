(function(){
  ...
  async function fetchPartial(paths){for(const p of paths){try{const r=await fetch(p,{cache:"no-cache"});if(r.ok)return await r.text()}catch(e){}}throw new Error("partials/nav.html not found")}
  ...
  async function init(){
    ...
    const candidates=["partials/nav.html","../partials/nav.html","../../partials/nav.html",SITE_BASE?SITE_BASE+"/partials/nav.html":null].filter(Boolean);
    const html=await fetchPartial(candidates); mount.innerHTML=html; setLinksAndActive(mount);
  }
